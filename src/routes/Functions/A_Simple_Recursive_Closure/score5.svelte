...
var l = m0(ret).map(v => v(ret))
console.log("m0(ret) is", l);
*/
...
function fg (x) {
	...
	runRoll = (b) => {
		...
		m2 = M(arr);
        m1(v => v = v.concat(arr));
        console.log("*****^^^^^***** In runRoll. m1(ret) is", m1(ret));
        console.log("*****^^^^^***** In runRoll. m2(ret) is", m2(ret));
		// m2 = m2(fuu);
    ...
    var fuDem = `var update = function update() {
		AA = m2(s)[0][0];
		BB = m2(s)[0][1];
		CC = m2(s)[0][2];
		DD = m2(s)[0][3];
	...
	$: sfunc = () => {
		...
		m2(v => v.filter(c => c != "undefined"));
		console.log("m2(ret) now is", m2(ret));
		if (m2(ret)[0].length < 4) b3 = b7 = "none";
    	if (m2(ret)[0].length < 3) b2 = b6 = "none";
		if (m2(ret)[0].length < 2) b1 = b5 = "none";
		if (m2(ret)[0].length < 1) b0 = b4 = "none";
	}
...
*/
	...
	function test() {
		...
		console.log(m2(s)[0].join(", "));
        console.log("-O-O-OO->  m2(ret)[0][0].length is", m1(ret)[0][0].length);
		console.log("$O$O$O$O$O b0,b1,b2,b3,b4,b,5,b6,b7", b0,b1,b2,b3,b4,b,5,b6,b7)      
        console.log("->  m1(ret) is", m1(ret));
        // console.log("----->---->--->--> m1(ret)[4] is", m1(ret)[4], "<---<--<OO>");
	}

	function fu(a) {
		// a[7].push(clone(a));
		a[6][0] += 1;
        // m1(v => {x = clone(v); x.push(a); return x;});
        m1(x => x = x.concat(clone([a])));
        console.log("*****$$$$$$***** In fu. m1(ret) is", m1(ret));
        index = a[6][0];
		// a[1].push(a[0].pop());
		var result;
		var a1;
		...
		sfunc();
        // m1(v => {x = clone(v); x.push(a); return x;});
        console.log("*****OOOOO***** Bottom of fu. m1(ret) is", m1(ret));
		return m2(s);
	}
...
<br /><br />
<button on:click={() => m1(backOne)}>TAKE BACK MOVE</button>
<br><br>
