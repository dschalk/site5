...
var runRoll;
runRoll = x => a => {
     x = M([ [Math.floor(Math.random()*6) + 1, Math.floor(Math.random()*6) + 1, Math.floor(Math.random()*12) + 1, Math.floor(Math.random()*20) + 1], [], [], [], [0], [], [0] ]);
    updateRoll(x);
    EEE = "yet to be selected";
    XO = x('stop');
    console.log("In runRoll. XO IS",XO);
    return x;
}
...
function sfunc () {
    ...
    if (m2(x)[0][0] != (undefined && 0)) b0 = "inline";
    if (m2(x)[0][1] != (undefined && 0)) b1 = "inline";
    if (m2(x)[0][2] != (undefined && 0)) b2 = "inline";
    if (m2(x)[0][3] != (undefined && 0)) b3 = "inline";
    if (m2(x)[1][0] != (undefined && 0)) b4 = "inline";
    if (m2(x)[1][1] != (undefined && 0)) b5 = "inline";
    if (m2(x)[1][2] != (undefined && 0)) b6 = "inline";
    if (m2(x)[1][3] != (undefined && 0)) b7 = "inline";
}
...
var click0 = a => {
    console.log("a is", a);
    a[1].push(a.splice(0,1)[0]);
    console.log("in click0. a(s) is", a(s));
};
...
<br><br>
<button on:click = {() => m2(runRoll(SCORE))}>ROLL</button>
<br><br>

<div>XO is: {XO}</div>
<!--<div>XO[0] is: {XO[0]}</div>
<div>selected: {XO[1]}</div>
<div>operator: {XO[2]}</div>
<div>computed: {XO[3]}</div>
<div>score: {XO[4]}</div>
<div>archive: {XO[5]}</div>
<div>index: {XO[6]}</div>-->
<br>
