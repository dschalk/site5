...
<h2>Functional Programming Paradigm Ignored</h2>
 ...
 <p>Some Haskell programmers behave like dour monks in an austere monistary, always ready to downvote advice that deviates from Haskell's "no side effects, no mutations" ethos. Some fanatical functional JavaScript coders won't even mutate variables confined entirely in function scope.  </p>
...
<p>The third time "m(()=>4)" executes, a process runs placing a new item in RESULT's head, and incrementing the index of each RESULT property. Since RESULT.length = 6 is maintained, the previous tail is abandoned to the garbage collector            .</p>
<pre>{change}</pre> 
<pre>{code1}</pre>
<br>
